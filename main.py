...
def parse_arguments():
    ...
    parser.add_argument("-s", "--mad-cutoff", type=int, default=9, help="insert size cutoff, median+s*MAD (deletions only)")
    ...
    parser.add_argument("-a", "--max-geno-count", type=int, default=250, help="max. number of reads aligned for SR genotyping")
...
def main():
    ...
    if not parseExcludeIntervals(c, samfile, validRegions):
    ...
    if not c.hasVcfFile:
        ...
        scanPE(c, validRegions, svs, srSVs, srStore, sampleLib)
    ...
    read_count_array = [[ReadCount(random.randint(0, 10), random.randint(0, 10), random.randint(0, 10)) for _ in range(2)] for _ in range(2)]
    spanning_count_array = [[SpanningCount() for _ in range(2)] for _ in range(2)]

    vcf_output(c, svs, jctMap, read_count_array, spanning_count_array)
